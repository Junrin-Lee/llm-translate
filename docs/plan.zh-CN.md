...
- **一期 —— 已交付**:可注入页面上,在页面的冻结截图上拖拽框选区域,译文以译文卡片原地流式输出;受限页面(内置 PDF 阅读器、浏览器内部页面)自动降级为一个扩展页,该页同时接受粘贴/拖入/选择的图片;入口为 popup 与右键菜单;通过既有功能级覆盖链(划词/全文/图片)解析 Provider,未设置则跟随全局默认 Provider,要求模型支持图片输入(非视觉模型报错会深链到路由设置);`ChatRequest` 与两个协议 adapter 均已支持多模态;首次使用有一次性隐私提示;图片翻译一期不做缓存。
